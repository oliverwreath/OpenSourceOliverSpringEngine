...
class CourseServiceTests extends ServiceTestAbstract<Course, Long> {
    @Autowired
    private CourseService service;

    @Override
    protected Course getTestEntity() {
        return new Course(TEST_STRING_1);
    }
    ...
    public void givenCRUD_whenAdded_thenFound_whenUpdate_thenMatch_whenDelete_thenGone() {
        // C write
        Course persistedEntity = service.saveOrReplace(getTestEntity());
        Long id = persistedEntity.getId();
        // exists
        assertThat(service.existsById(id)).isTrue();
        // R read
        Optional<Course> foundEntityOptional = service.findById(persistedEntity.getId());
        assertThat(foundEntityOptional).isNotNull();
        assertThat(foundEntityOptional.isPresent()).isTrue();
        // same
        Course foundEntity = foundEntityOptional.get();
